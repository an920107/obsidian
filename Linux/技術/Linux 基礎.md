...
- Linux 檔案結構是一個 tree，這個 tree 的 root 稱為 **根目錄 (/)**
	...
	![|500](https://i.imgur.com/vyGJeTL.png)
...
- `pwd`：*print work directory*，印出當前目錄的絕對路徑
	...
	![|500](https://i.imgur.com/rbmC1o4.png)

- `ls`：*list*，印出有哪些檔案或目錄（不包含隱藏檔案目錄），後方可以輸入目標目錄，當只輸入 `ls` 時，視為當下的目錄
	
	![|500](https://i.imgur.com/qdNoDPy.png)
    - `ls -l`：以詳細資訊列表印出，在 Ubuntu 中 `ll` 指令可達到相同效果
	      
		![|500](https://i.imgur.com/sMGjYVF.png)
    
    - `ls -a`：印出所有檔案目錄，包含隱藏檔案目錄 (以 "." 開頭的檔案目錄)
	      
		![|500](https://i.imgur.com/tdRjiIF.png)
    
    - 以上兩者可混用 `ls -la`
	      
	    ![|500](https://i.imgur.com/duXhkf2.png)

- `cd`：*change directory*，更換目錄到指定路徑，可以是相對或絕對路徑，當後方完全沒有路徑時，將回到家目錄
	  
	![|500](https://i.imgur.com/qmWH8Fs.png)

- `mkdir`：*make directory*，建立新目錄，後方輸入新目錄名稱
	  
	![|500](https://i.imgur.com/i9kXVFH.png)

- `cp`：*copy*，複製檔案，先輸入原檔案路徑，再輸入新檔案路徑
    - 只複製檔案
        - 同一個路徑
	          
	        ![|500](https://i.imgur.com/RXBiwp1.png)

        - 不同路徑
	          
	        ![|500](https://i.imgur.com/aE2v7nd.png)
    - `cp -r`：複製整個資料夾，以及裡面的所有東西
	      
		![|500](https://i.imgur.com/Tc24Ajn.png)

- `mv`：*move*，移動檔案或目錄（也可當作重新命名），先輸入原檔案路徑，再輸入新檔案路徑
    - 移動
	    
	    ![|500](https://i.imgur.com/XbeDbJf.png)
      
    - 重新命名
	      
	    ![|500](https://i.imgur.com/Q44Pymz.png)

- `rm`：*remove*，刪除指定檔案，不會進到資源回收桶，一去不復返
    - 刪除檔案
    
	    ![|500](https://i.imgur.com/FsYysrZ.png)
    
    - `rm -r`：刪除整個目錄（謹慎操作，一去不復返）
	      
	    ![|500](https://i.imgur.com/Xja3v6f.png)

- `cat`：印出指定的檔案內容
	  
	![|500](https://i.imgur.com/9Jt3fik.png)
...
- nano
	...
	![|500](https://i.imgur.com/pKk5rxU.png)
    - 操作上較直覺
    - 下方的快捷鍵列表，^ 表示 Ctrl 鍵，例如按下 Ctrl + X 可離開編輯器
- vim
	  
	![|500](https://i.imgur.com/PdxfujD.png)
    - 極度新手不友善
...
- 使用 `sudo` (*super user do*) 可一次性的以超級管理員執行後面的指令
	...
	![|500](https://i.imgur.com/Jw2nxkE.png)
...
- 使用 `ls -l` 可以看到檔案的權限設置
	...
	![|500](https://i.imgur.com/DDg2cGP.png)
    - 第一欄位 `drwxr-x---`
      ...
      ![|500](https://i.imgur.com/Frc7LYw.png)
    - 第二欄位 數字
        - 如果是目錄，顯示該目錄底下的項目數量
        - 其他類型，此數字為 1
    - 第三欄位 `root` `an920107`
...
- `man`：*manual*，後面輸入指令，可以查詢該指令的使用方式（雖然多數時候 Google 可能比較好用）
	...
	![|500](https://i.imgur.com/j2gzWii.png)
	  
	![|500](https://i.imgur.com/jbts2QE.png)
